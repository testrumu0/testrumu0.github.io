...
%>
   <div class="wrap">
     <h3 class="header"><%=suserid%>님의 일정</h3><br>
        <table class="view_table">
		<form name="delete_form" method="post" action="schedule_delete.jsp?idx=<%=idx%>">
            <tr>
                <th width="80px;" height="25px;">글번호</th>
                <td>
                <%=idx%>
                </td>
            </tr>
			<tr>
                <th width="80px;" height="25px;">제목</th>
                <td>
                <%=stitle%>
                </td>
            </tr>
			<tr>
                <th width="80px;" height="25px;">알림</th>
                <td>
                <%=salarm%>
                </td>
            </tr>
            <tr>
                <th width="80px;" height="25px;">기간</th>
                <td>
                <%=syear+"."+smonth+"."+sday%>
                </td>
            </tr>
			<tr>
                <th width="80px;" height="20px;">시간</th>
                <td>
                <%=time_start + "시 - " + time_end + "시"%>
                </td>
            </tr>
            <tr>
                <th width="80px;" height="20px;">내용</th>
                <td>
                <%=scontent%>
                </td>
            </tr>
...
<br>
   <div align ="center">
    <input type="button" value="수정"  onclick="location.href='schedule_min_update.jsp?idx=<%=idx%>' ">
	<input type="button" value="삭제" onclick="location.href='schedule_min_delete.jsp?idx=<%=idx%>' ">
   </div>
  </div><!--wrap--> 
        <%	
				rs.close();
